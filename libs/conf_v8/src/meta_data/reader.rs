...
// #include "Logger.h"
// #include "IConfigurationBlock.h"

// /**
// * Выполняет модификацию исходного cf файла
// *
// * @param Фильтр метаданных, который необходимо применить при сборке нового cf файла
// */
// void Metadata::modifyStructure(const MetadataFiltersList &filter) {

//     bool hasUnitAssembly = false;
//     for (auto it = filter.begin(); it != filter.end(); it++) {
//         hasUnitAssembly = it->refersToUnitAssembly;
//         if ( hasUnitAssembly ) {
//             break;
//         }
//     }
...
//     if ( !hasUnitAssembly ) {
//         BasicException("Настройки фильтрации метаданных не содержат информации об основных объектах сборки.");
//     }

//     IConfigurationBlock *confStructBlock = confFile->findBlock( getConfingurationId() );
//     std::string confStruct = confStructBlock->getText();

//     TypesList typesList;
//     LOG_OUT_INFO(LoggerObject, "Чтение структуры конфигурации");
//     readConfingurationStruct(&typesList, confStruct, filter);
//     LOG_OUT_INFO(LoggerObject, "-Чтение структуры конфигурации");

//     LOG_OUT_INFO(LoggerObject, "Удаление объектов метаданных");
//     removeObjetsFiles(typesList);
//     LOG_OUT_INFO(LoggerObject, "-Удаление объектов метаданных");

//     LOG_OUT_INFO(LoggerObject, "Замена ссылок удаленных объектов метаданных");
//     removeIdsDeletedObjects(typesList);
//     LOG_OUT_INFO(LoggerObject, "-Замена ссылок удаленных объектов метаданных");

//     LOG_OUT_INFO(LoggerObject, "Корректировка состава элементов");
//     correctConfStruct(&confStruct, typesList);
//     LOG_OUT_INFO(LoggerObject, "-Корректировка состава элементов");

//     confStructBlock->setText(confStruct);
// }

// void Metadata::correctConfStruct(std::string *confStruct, const TypesList &typesList)
// {
//     std::stringstream typeComposition;

//     for (auto& typeDescription:typesList) {
//         int countObjects = 0;
//         typeComposition.str("");

// for (auto& objectDescription:typeDescription.objects) {

//             if ( objectDescription.collect ) {
//                 typeComposition << (countObjects == 0 ? "" : ",") << objectDescription.id;
//                 countObjects++;
//             }
//         }
...
//         if ( countObjects == 0 ) {
//             newTypeText = bs::format( "{0},0", {typeDescription.id} );
//         } else {
//             newTypeText = bs::format( "{0},{1},{2}", {typeDescription.id, convert::to_string(countObjects), typeComposition.str()} );
//         }
//         TextCoordinates coordinates = findTypeCoordinates(typeDescription.id, *confStruct);
//         confStruct->replace( coordinates.start, coordinates.length(), newTypeText.c_str() );
//     }
// }

// void Metadata::readConfingurationStruct( TypesList *typesList,
//                                          std::string &confStruct,
//                                          const MetadataFiltersList &filter)
// {
//     const StringsMap types = getTypes();

//     for ( auto& typeIdName:types) {
//         const std::string &typeId = typeIdName.first;

//         TextCoordinates coordinates = findTypeCoordinates(typeId, confStruct);
//         if ( coordinates.empty() )
//             continue;

//         StringsVector objectsIds;
//         extractObjetsIdsForType(&objectsIds, confStruct, coordinates);
//         if ( objectsIds.empty() )
//             continue;

//         typesList->resize( typesList->size() +1);
//         TypeDescription &typeDescription = (*typesList)[ typesList->size()-1 ];
//         typeDescription.id = typeId;
//         typeDescription.objects.reserve( objectsIds.size() );

//         MetadataFiltersList typeFilter;
//         for (auto& object:filter) {
//             if ( object.typeId == typeId ) {
//                 typeFilter.push_back(object);
//             }
//         }

//         for (auto& currentId:objectsIds) {
//             ObjectDescription objectDescription(currentId);
//             completeObjectDescription(&objectDescription, typeFilter);

//             typeDescription.objects.push_back(objectDescription);
//         }
//     }
// }

// /**
// * Удаляет файлы объектов сборки которые не прошли по фильтру
// *
// * @param структура конфигурации
// */
// void Metadata::removeObjetsFiles(const TypesList &typesList)
// {
//     for (auto& typeDescription:typesList) {
//         for (auto& objectDescription:typeDescription.objects) {

//             if ( !objectDescription.collect ) {
//                 confFile->deleteBlock(objectDescription.id);
//             }

//             if ( !objectDescription.collect || !objectDescription.refersToUnitAssembly ) {
//                 for ( auto &internalElement:objectDescription.internalElements ) {
//                     for ( auto &id:internalElement.second ) {

//                         confFile->deleteBlock(id);
//                     }
//                 }
//             }
//         }
...
// /**
// * Удаляет ссылки на удаленные объекты
// *
// * @param структура конфигурации
// */
// void Metadata::removeIdsDeletedObjects(const TypesList &typesList)
// {
//     LOG_OUT_INFO(LoggerObject, "d1");

// std::set< std::string > idsDeletedObjects;

//     for (auto& typeDescription:typesList) {
//         for (auto& objectDescription:typeDescription.objects) {

//             if ( !objectDescription.collect ) {
//                 idsDeletedObjects.insert(objectDescription.refsIds.begin(), objectDescription.refsIds.end());
//             }
//         }
//     }
...
//             StringsVector ids = confFile->findNamesIndependentBlocks(objectDescription.id);
//             ids.push_back(objectDescription.id);

//             for (auto& blockId:ids) {
//                 IConfigurationBlock* block = confFile->findBlock( blockId );
//                 if ( block->isModule() ) {
//                     continue;
//                 }

// const char *subBlockName = block->isForm() ? "form" : "";

//                 std::string text = block->getText(subBlockName);
//                 replaceDeletedIds(&text, idsDeletedObjects);

// if ( !objectDescription.refersToUnitAssembly ) {

//                     for ( auto &internalElement:objectDescription.internalElements ) {
//                         const std::string &internalTypeId = internalElement.first;
//                         TextCoordinates coordinates = findTypeCoordinates(internalTypeId, text);
//                         text.replace(coordinates.start, coordinates.length(), (internalTypeId +",0").c_str() );
//                     }
//                 }

//                 block->setText(text, subBlockName);
//             }
//         }
//     }

//     LOG_OUT_INFO(LoggerObject, "d2");
// }

// bool Metadata::replaceDeletedIds(std::string * text, const std::set< std::string > &idsDeletedObjects)
// {
//     bool haveReplacement = false;

// const char *startRefText = "{\"#\",";

//     size_t startPos(-1), endPos(-1);
//     while ( !text->empty() ) {
//         startPos = text->find(startRefText, startPos+1);
...
//         coordinates.start = startPos + strlen(startRefText);
//         coordinates.end = endPos-1;

//         const std::string refVal = text->substr(coordinates.start, coordinates.length());
//         auto it = idsDeletedObjects.find(refVal);
//         if ( it != idsDeletedObjects.end() ) {
//             text->replace(coordinates.start, coordinates.length(), ANY_REF);
