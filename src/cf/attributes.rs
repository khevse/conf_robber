...
impl Attributes {
    ...
    pub fn new(_creation_date: u64, _group_type: i32, _name: &String) -> Attributes {
    ...
    pub fn from_cf(data: &[u8]) -> Attributes {
        let end_name = data.len() - 4;
        let name = match String::from_utf8(conv::utf16_to_utf8(&data[20..end_name])) {
            Ok(v) => v,
            Err(e) => {
                error!("Failed to get the name of the block: {}", e);
                panic!("Failed to get the name of the block: {}", e);
            }
        };
    ...
    pub fn for_cf(&self) -> Vec<u8> {
        ...
        let unknow_block: [u8; 4] = match self.name.len() {
        ...
        data.extend_from_slice(&conv::utf8_to_utf16(&self.name.as_bytes()[..]));
